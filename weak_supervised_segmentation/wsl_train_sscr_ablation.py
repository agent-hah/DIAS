...
class Trainer:
    ...
    def _valid_epoch(self, epoch):
        ...
        log = {"val_loss": self.total_loss.mean, **self._metrics_ave()}
        return log
...
def main_worker(local_rank, config):
    ...
    if config.DIS:
        model1 = torch.nn.parallel.DistributedDataParallel(
            model1, device_ids=[local_rank], find_unused_parameters=True
        )
        model2 = torch.nn.parallel.DistributedDataParallel(
            model2, device_ids=[local_rank], find_unused_parameters=True
        )
    logger.info(f"\n{model1}\n")
