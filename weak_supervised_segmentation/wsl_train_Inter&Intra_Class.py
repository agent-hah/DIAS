...
def intra_class_variance(prob, img):
    mean_std = torch.std(
        torch.min(img, dim=1, keepdim=True)[1].float() * prob, dim=[2, 3]
    )
...
def inter_class_variance(prob, img):
    mean_std = torch.std(
        torch.mean(torch.min(img, dim=1, keepdim=True)[1].float() * prob, dim=[2, 3]),
        dim=1,
    )
    return mean_std.mean()
        ...
        for idx, (img, gt) in enumerate(tbar):
            ...
            with torch.amp.autocast("cuda", enabled=self.config.AMP):
                ...
                consistency_loss = inter_class_variance(
                    outputs_soft, img
                ) - intra_class_variance(outputs_soft, img)
            ...
            self._metrics_update(
                **get_metrics(
                    torch.softmax(pre[:, : self.config.DATASET.NUM_CLASSES], dim=1)
                    .cpu()
                    .detach()
                    .numpy()[:, 1, :, :],
                    gt.cpu().detach().numpy(),
