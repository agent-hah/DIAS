...
from utils.helpers import to_cuda, target_device, load_checkpoint
...
class Trainer:
    def __init__(
        ...
        self.scaler = torch.amp.GradScaler("cuda", enabled=(str(self.device) == "cuda"))
        self.is_2d = is_2d
        self.model1 = model1
        self.model2 = model2
        ...
        self.pce_loss = CrossEntropyLoss(ignore_index=255).to(self.device)
        self.ce_loss = CrossEntropyLoss().to(self.device)
        self.con_loss = MSELoss().to(self.device)
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.config.TRAIN.EPOCHS + 1):
            ...
            # SAVE CHECKPOINT
            if self._get_rank() == 0:
                self._save_checkpoint(epoch, save_best=bool(self.improved))
    ...
    def _train_epoch(self, epoch):
        ...
        for idx, (img1, img2, gt) in enumerate(tbar):
            ...
            with torch.autocast(
                ...
                pseudo_gt1 = torch.argmax(
                    torch.softmax(pre1[:, :-1, :, :], dim=1), dim=1, keepdim=False
                )
                pseudo_gt2 = torch.argmax(
                    torch.softmax(pre2[:, :-1, :, :], dim=1), dim=1, keepdim=False
                )
                pseudo_gt1[gt == 0] = 0
                pseudo_gt1[gt == 1] = 1
                pseudo_gt2[gt == 0] = 0
                ...
                loss = (
                    pce_loss1
                    + pce_loss2
                    + self.pseudo_weight * (ce_loss1 + ce_loss2)
                    + self.consistency_weight * (self.consistency_loss)
                )
                # loss = pce_loss1+pce_loss2

            if self.config.AMP and str(self.device) == "cuda":
                self.scaler.scale(loss).backward()
                if self.config.TRAIN.DO_BACKPROP:
                    torch.nn.utils.clip_grad_norm_(self.model1.parameters(), 12)
            ...
            self._update_metrics(
                **get_metrics(
                    torch.softmax(pre1[:, : self.config.DATASET.NUM_CLASSES], dim=1)
                    .cpu()
                    .detach()
                    .numpy()[:, 1, :, :],
                    gt.cpu().detach().numpy(),
        ...
        with torch.no_grad():
            for idx, (img, gt) in enumerate(tbar):
                ...
                self._update_metrics(
                    **get_metrics(
                        torch.softmax(
                            predict[:, : self.config.DATASET.NUM_CLASSES], dim=1
                        )
                        .cpu()
                        .detach()
                        .numpy()[:, 1, :, :],
...
def main_worker(local_rank, config):
    ...
    if config.DIS:
        ...
        dist.init_process_group(
            backend, init_method="env://", rank=local_rank, world_size=config.WORLD_SIZE
        )
