...
from utils.helpers import to_cuda, target_device
...
class Trainer:
    def __init__(self, config, train_loader, val_loader, is_2d, model1, model2, optimizer1, optimizer2, lr_scheduler1, lr_scheduler2):
        ...
        self.scaler = torch.cuda.amp.GradScaler(enabled=(str(self.device) == 'cuda'))
        self.is_2d = is_2d
        self.model1 = model1
        self.model2 = model2
    ...
    def _train_epoch(self, epoch):
        ...
        for idx, (img1, img2, gt) in enumerate(tbar):
            ...
            with torch.autocast(device_type=str(self.device), dtype=torch.float16, enabled=self.config.AMP):
...
def main_worker(local_rank, config):
    ...
    if config.DIS:
        model1 = torch.nn.parallel.DistributedDataParallel(
            model1.to(device), device_ids=[local_rank] if device.type == 'cuda' else None, find_unused_parameters=True)
        model2 = torch.nn.parallel.DistributedDataParallel(
            model2.to(device), device_ids=[local_rank] if device.type == 'cuda' else None, find_unused_parameters=True)
    else:
