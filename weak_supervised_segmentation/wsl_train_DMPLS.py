    ...
    def _train_epoch(self, epoch):
        ...
        for idx, (img, gt) in enumerate(tbar):
            ...
            with torch.amp.autocast("cuda", enabled=self.config.AMP):
                ...
                loss_pse_sup = 0.5 * (
                    self.dice_loss(outputs_soft1, pseudo_supervision.unsqueeze(1))
                    + self.dice_loss(outputs_soft2, pseudo_supervision.unsqueeze(1))
                )
            ...
            self._metrics_update(
                **get_metrics(
                    torch.softmax(outputs[:, : self.config.DATASET.NUM_CLASSES], dim=1)
                    .cpu()
                    .detach()
                    .numpy()[:, 1, :, :],
                    gt.cpu().detach().numpy(),
        ...
        with torch.no_grad():
            for idx, (img, gt) in enumerate(tbar):
                ...
                self._metrics_update(
                    **get_metrics(
                        torch.softmax(
                            outputs[:, : self.config.DATASET.NUM_CLASSES], dim=1
                        )
                        .cpu()
                        .detach()
                        .numpy()[:, 1, :, :],
...
def parse_option():
    ...
    parser.add_argument(
        "--resume", type=str, help="Path to the save folder to resume training from"
    )
...
def main_worker(local_rank, config):
    ...
    if config.TRAIN.RESUME_PATH:
        ...
        checkpoint = load_checklpoint(config.TRAIN.RESUME_PATH, is_best=False)
