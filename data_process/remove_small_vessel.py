...
# # Define the maximum width (in pixels) of small blood vessels to be removed
# max_vessel_width = 1
...
# # Use distance transformation to determine the distance of each pixel to the nearest background pixel
# dist_transform = cv2.distanceTransform(blood_vessel_image, cv2.DIST_L2, 3)

# # Determine the area of small blood vessels based on the distance transformation result
# small_vessels = (dist_transform < max_vessel_width).astype(np.uint8)

# # Remove small blood vessels from the original image
...
# print("Processing completed, the image has been saved to the output folder.")
...
for file in files:
    ...
    # Perform morphological operations to remove small blood vessels
